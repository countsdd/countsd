...
Clock::Clock(int day, int month, int year, int hours, int mins, string day_of_week, vector<string> tasks) 
    : day_(day), month_(month), year_(year), hours_(hours), mins_(mins), 
      day_of_week_(day_of_week), tasks_(tasks), battery_(100, 100) {
...
Clock::Clock(const Clock &p) 
    : day_(p.day_), month_(p.month_), year_(p.year_), 
      ...
      tasks_(p.tasks_), battery_(p.battery_) {
...
void Clock::add_min(int minutes) {
    ...
    if (battery_.getCurrentCharge() <= 0) {
        cout << "ВНИМАНИЕ: Батарейка разряжена! Время не может быть добавлено." << endl;
        return;
    }
    
    int total_minutes = mins_ + minutes;
    int extra_hours = total_minutes / 60;
    int extra_days = extra_hours / 24;
    
    if (extra_days > 0) {
        if (battery_.getCurrentCharge() < extra_days) {
            cout << "ВНИМАНИЕ: Недостаточно заряда батарейки! Будет добавлено только " 
                 << battery_.getCurrentCharge() << " дней." << endl;
            extra_days = battery_.getCurrentCharge();
            minutes = extra_days * 24 * 60; 
        }
        
        battery_.discharge(extra_days);
    }
    
    mins_ += minutes;
    extra_hours = mins_ / 60;
    mins_ = mins_ % 60;
...
void Clock::print_info() const {
    ...
    cout << endl;
    cout << "Заряд батарейки: " << battery_.getCurrentCharge() << "/" << battery_.getMaxCharge() << endl;
    cout << "=============" << endl;
}
...
void Clock::set_battery(int current, int max) {
    battery_.setCharge(current);
    battery_.setMaxCharge(max);
}
